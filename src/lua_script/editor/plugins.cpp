...
struct PropertyGridPlugin MALMY_FINAL : public PropertyGrid::IPlugin
{
	struct AddLuaScriptCommand MALMY_FINAL : public IEditorCommand
	{
	...
	struct MoveScriptCommand MALMY_FINAL : public IEditorCommand
	{
	...
	struct RemoveScriptCommand MALMY_FINAL : public IEditorCommand
	{
	...
	struct SetPropertyCommand MALMY_FINAL : public IEditorCommand
	{
...
struct ConsolePlugin MALMY_FINAL : public StudioApp::GUIPlugin
{
	...
	{
		Action* action = MALMY_NEW(app.getWorldEditor().getAllocator(), Action)("Script Console", "Toggle script console", "script_console");
	...
	{
		LuaScriptScene* scene = (LuaScriptScene*)app.getWorldEditor().getProject()->getScene(crc32("lua_script"));
...
{
	return MALMY_NEW(editor.getAllocator(), PropertyGridPlugin::AddLuaScriptCommand)(editor);
}


IEditorCommand* createSetPropertyCommand(WorldEditor& editor)
{
	return MALMY_NEW(editor.getAllocator(), PropertyGridPlugin::SetPropertyCommand)(editor);
}


IEditorCommand* createRemoveScriptCommand(WorldEditor& editor)
{
	return MALMY_NEW(editor.getAllocator(), PropertyGridPlugin::RemoveScriptCommand)(editor);
}


struct AddComponentPlugin MALMY_FINAL : public StudioApp::IAddComponentPlugin
{
...
struct StudioAppPlugin : StudioApp::IPlugin
{
		...
		: m_app(app)
	{
		...
		m_add_component_plugin = MALMY_NEW(allocator, AddComponentPlugin)(app);
		...
		m_prop_grid_plugin = MALMY_NEW(allocator, PropertyGridPlugin)(app);
		app.getPropertyGrid().addPlugin(*m_prop_grid_plugin);

		m_asset_browser_plugin = MALMY_NEW(allocator, AssetBrowserPlugin)(app);
		app.getAssetBrowser().addPlugin(*m_asset_browser_plugin);

		m_console_plugin = MALMY_NEW(allocator, ConsolePlugin)(app);
	...
	~StudioAppPlugin()
	{
		...
		MALMY_DELETE(allocator, m_prop_grid_plugin);

		m_app.getAssetBrowser().removePlugin(*m_asset_browser_plugin);
		MALMY_DELETE(allocator, m_asset_browser_plugin);

		m_app.removePlugin(*m_console_plugin);
		MALMY_DELETE(allocator, m_console_plugin);
	}
