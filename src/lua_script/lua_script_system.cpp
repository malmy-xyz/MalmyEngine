...
namespace Malmy
{
	...
	class LuaScriptSystemImpl MALMY_FINAL : public IPlugin
	{
	public:
	...
	struct LuaScriptSceneImpl MALMY_FINAL : public LuaScriptScene
	{
		struct TimerData
		{
			float time;
	...
	public:
		LuaScriptSceneImpl(LuaScriptSystemImpl& system, Project& ctx)
			: m_system(system)
			, m_project(ctx)
		...
		void clear() override
		{
			...
			for (auto* script_cmp : m_scripts)
			{
				...
				MALMY_DELETE(m_system.m_allocator, script_cmp);
			}
			m_scripts.clear();
		}
		...
		struct GetPropertyVisitor MALMY_FINAL : Reflection::IPropertyVisitor
		{
			template <typename T>
			MALMY_FORCE_INLINE void get(const Reflection::Property<T>& prop)
			{
				T v;
		...
		struct SetPropertyVisitor MALMY_FINAL : Reflection::IPropertyVisitor
		{
			template <typename T>
			MALMY_FORCE_INLINE void set(const Reflection::Property<T>& prop)
			{
		...
		, m_script_manager(m_allocator)
	{
		...
		static auto lua_scene = scene("lua_script",
			component("lua_script",
				blob_property("data", MALMY_PROP(LuaScriptScene, ScriptData))
			)
		);
	...
	void LuaScriptSystemImpl::createScenes(Project& ctx)
	{
		auto* scene = MALMY_NEW(m_allocator, LuaScriptSceneImpl)(*this, ctx);
		ctx.addScene(scene);
	}

	void LuaScriptSystemImpl::destroyScene(IScene* scene)
	{
		MALMY_DELETE(m_allocator, scene);
	}

	MALMY_PLUGIN_ENTRY(lua_script)
	{
		return MALMY_NEW(engine.getAllocator(), LuaScriptSystemImpl)(engine);
	}
}
