...
struct SpritePlugin MALMY_FINAL : public AssetBrowser::IPlugin
{
...
class GUIEditor MALMY_FINAL : public StudioApp::GUIPlugin
{
enum class EdgeMask
{
	LEFT = 1 << 0,
...
public:
	GUIEditor(StudioApp& app)
	{
		...
		Action* action = MALMY_NEW(allocator, Action)("GUI Editor", "Toggle gui editor", "gui_editor");
		...
		m_editor->projectCreated().bind<GUIEditor, &GUIEditor::onProjectChanged>(this);
		m_editor->projectDestroyed().bind<GUIEditor, &GUIEditor::onProjectChanged>(this);
	}


	~GUIEditor()
	{
		m_editor->projectCreated().unbind<GUIEditor, &GUIEditor::onProjectChanged>(this);
		m_editor->projectDestroyed().unbind<GUIEditor, &GUIEditor::onProjectChanged>(this);
...
private:
	...
	void onProjectChanged()
	{
		...
		RenderScene* scene = (RenderScene*)project->getScene(crc32("renderer"));
	...
	static Vec2 toMalmy(const ImVec2& value)
	{
	...
	void onWindowGUI() override
	{
		if (ImGui::BeginDock("GUIEditor", &m_is_window_open))
		{
			...
			if (m_editor->getSelectedEntities().size() == 1)
			{
				has_rect = m_editor->getProject()->hasComponent(m_editor->getSelectedEntities()[0], GUI_RECT_TYPE);
			}
			if (has_rect && ImGui::BeginPopupContextItem("context"))
			{
				...
				if (ImGui::BeginMenu("Make relative"))
				{
					...
					if (ImGui::MenuItem("Top")) makeRelative(e, toMalmy(size), (u8)EdgeMask::TOP);
					if (ImGui::MenuItem("Right")) makeRelative(e, toMalmy(size), (u8)EdgeMask::RIGHT);
					if (ImGui::MenuItem("Bottom")) makeRelative(e, toMalmy(size), (u8)EdgeMask::BOTTOM);
					if (ImGui::MenuItem("Left")) makeRelative(e, toMalmy(size), (u8)EdgeMask::LEFT);
					
					ImGui::EndMenu();
				}
				if (ImGui::BeginMenu("Make absolute"))
				{
					if (ImGui::MenuItem("All")) makeAbsolute(e, toMalmy(size), (u8)EdgeMask::ALL);
					if (ImGui::MenuItem("Top")) makeAbsolute(e, toMalmy(size), (u8)EdgeMask::TOP);
					if (ImGui::MenuItem("Right")) makeAbsolute(e, toMalmy(size), (u8)EdgeMask::RIGHT);
					if (ImGui::MenuItem("Bottom")) makeAbsolute(e, toMalmy(size), (u8)EdgeMask::BOTTOM);
					if (ImGui::MenuItem("Left")) makeAbsolute(e, toMalmy(size), (u8)EdgeMask::LEFT);
					ImGui::EndMenu();
				}
...
struct StudioAppPlugin : StudioApp::IPlugin
{
		...
		: m_app(app)
	{
		...
		IAllocator& allocator = app.getWorldEditor().getAllocator();
		m_gui_editor = MALMY_NEW(allocator, GUIEditor)(app);
		app.addPlugin(*m_gui_editor);

		m_sprite_plugin = MALMY_NEW(allocator, SpritePlugin)(app);
	...
	~StudioAppPlugin()
	{
		...
		MALMY_DELETE(allocator, m_gui_editor);

		m_app.getAssetBrowser().removePlugin(*m_sprite_plugin);
		MALMY_DELETE(allocator, m_sprite_plugin);
	}
