...
class EngineImpl MALMY_FINAL : public Engine
{
public:
		...
		, m_working_dir(working_dir)
	{
		...
		if (!fs)
		{
			...
			m_mem_file_device = MALMY_NEW(m_allocator, FS::MemoryFileDevice)(m_allocator);
			m_resource_file_device = MALMY_NEW(m_allocator, FS::ResourceFileDevice)(m_allocator);
			m_disk_file_device = MALMY_NEW(m_allocator, FS::DiskFileDevice)("disk", working_dir, m_allocator);
			...
			if (is_patching)
			{
				m_patch_file_device = MALMY_NEW(m_allocator, FS::DiskFileDevice)("patch", base_path1, m_allocator);
	...
	static Project* LUA_getSceneProject(IScene* scene) { return &scene->getProject(); }
	...
	static int LUA_loadProject(lua_State* L)
	{
		...
		auto* project = LuaWrapper::checkArg<Project*>(L, 2);
		...
		struct Callback
		{
			...
			void invoke(FS::IFile& file, bool success)
			{
				if (!success)
				{
					g_log_error.log("Engine") << "Failed to open project " << path;
				}
				else
				{
					...
					if (!engine->deserialize(*project, blob))
					{
						g_log_error.log("Engine") << "Failed to deserialize project " << path;
					}
					else
					{
				...
				MALMY_DELETE(engine->getAllocator(), this);
			}
		...
		Callback* inst = MALMY_NEW(engine->getAllocator(), Callback);
		inst->engine = engine;
	...
	void registerLuaAPI()
	{
		...
		LuaWrapper::createSystemFunction(m_state, "Engine", "loadProject", LUA_loadProject);

		#undef REGISTER_FUNCTION

		#define REGISTER_FUNCTION(F) \
			do { \
				auto f = &LuaWrapper::wrapMethod<Project, decltype(&Project::F), &Project::F>; \
	...
	~EngineImpl()
	{
		...
		if (m_disk_file_device)
		{
			...
			MALMY_DELETE(m_allocator, m_mem_file_device);
			MALMY_DELETE(m_allocator, m_resource_file_device);
			MALMY_DELETE(m_allocator, m_disk_file_device);
			MALMY_DELETE(m_allocator, m_patch_file_device);
		}
	...
	void setPatchPath(const char* path) override
	{
		...
		if (!m_patch_file_device)
		{
			m_patch_file_device = MALMY_NEW(m_allocator, FS::DiskFileDevice)("patch", path, m_allocator);
			m_file_system->mount(m_patch_file_device);
			m_file_system->setDefaultDevice("memory:patch:disk:resource");
			m_file_system->setSaveGameDevice("memory:disk:resource");
		}
	...
	{
		Project* project = MALMY_NEW(m_allocator, Project)(m_allocator);
		const Array<IPlugin*>& plugins = m_plugin_manager->getPlugins();
		for (auto* plugin : plugins)
		{
			plugin->createScenes(*project);
		}
		...
		if (set_lua_globals)
		{
			...
			lua_pushlightuserdata(m_state, project);
			lua_setglobal(m_state, "g_project");
		}
	...
	void destroyProject(Project& project) override
	{
		...
		MALMY_DELETE(m_allocator, &project);
	...
	void startGame(Project& context) override
	{
