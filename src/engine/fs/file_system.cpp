...
namespace Malmy
{
	namespace FS
	{
		...
		class FSTask MALMY_FINAL : public MT::Task
		{
		public:
		...
		class FileSystemImpl MALMY_FINAL : public FileSystem
		{
		public:
				...
				, m_last_id(0)
			{
				...
				m_task = MALMY_NEW(m_allocator, FSTask)(&m_transaction_queue, m_allocator);
		...
		{
			return MALMY_NEW(allocator, FileSystemImpl)(allocator);
		}

		void FileSystem::destroy(FileSystem* fs)
		{
			MALMY_DELETE(static_cast<FileSystemImpl*>(fs)->getAllocator().getSourceAllocator(), fs);
		}
