...
namespace Malmy
{
	...
	public:
		...
		~PluginManagerImpl()
		{
			...
			{
				MALMY_DELETE(m_engine.getAllocator(), m_plugins[i]);
			}
		...
		void unload(IPlugin* plugin) override
		{
			int idx = m_plugins.indexOf(plugin);
			ASSERT(idx >= 0);
			MALMY_DELETE(m_engine.getAllocator(), m_plugins[idx]);
...
#error Unknown platform
			...
			if (lib)
			{
				...
				if (creator)
				{
					...
					if (!plugin)
					{
						...
						MALMY_DELETE(m_engine.getAllocator(), plugin);
						ASSERT(false);
					}
					else
					{
			...
			else
			{
				...
				//burda da hata geliyo
				g_log_warning.log("Core") << "Failed to load plugin. " << path_with_ext;
			}
			return nullptr;
		}
