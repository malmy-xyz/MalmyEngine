...
template< class T > typename constexpr RemoveReference<T>::Type&& Move(T&& t) { 
...
{
	using Subtuple = typename TupleElement<Index, const ::Malmy::Tuple<Types...> >::Tail;
...
{
	using Subtuple = typename TupleElement<Index, ::Malmy::Tuple<Types...> >::Tail;
	return (((Subtuple&)tuple).value);
}
