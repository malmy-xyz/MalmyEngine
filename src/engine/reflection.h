...
#define MALMY_PROP_FULL(Scene, Getter, Setter) \
	&Scene::Getter, #Scene "::" #Getter, &Scene::Setter, #Scene "::" #Setter
#define MALMY_PROP(Scene, Property) \
	&Scene::get##Property, #Scene "::get" #Property, &Scene::set##Property, #Scene "::set" #Property
#define MALMY_ENUM_VALUE(value) EnumValue(#value, (int)value)
#define MALMY_FUNC(Func)\
	&Func, #Func
...
namespace Malmy
{
	...
	namespace Reflection
	{
		...
		MALMY_ENGINE_API void init(IAllocator& allocator);
		MALMY_ENGINE_API void shutdown();

		MALMY_ENGINE_API int getScenesCount();
		MALMY_ENGINE_API const SceneBase& getScene(int index);

		MALMY_ENGINE_API int getEnumsCount();
		MALMY_ENGINE_API const EnumBase& getEnum(int index);

		MALMY_ENGINE_API const IAttribute* getAttribute(const PropertyBase& prop, IAttribute::Type type);
		MALMY_ENGINE_API void registerEnum(const EnumBase& e);
		MALMY_ENGINE_API void registerScene(const SceneBase& scene);
		MALMY_ENGINE_API const ComponentBase* getComponent(ComponentType cmp_type);
		MALMY_ENGINE_API const PropertyBase* getProperty(ComponentType cmp_type, const char* property);
		MALMY_ENGINE_API const PropertyBase* getProperty(ComponentType cmp_type, u32 property_name_hash);
		MALMY_ENGINE_API const PropertyBase* getProperty(ComponentType cmp_type, const char* property, const char* subproperty);

		MALMY_ENGINE_API ComponentType getComponentType(const char* id);
		MALMY_ENGINE_API u32 getComponentTypeHash(ComponentType type);
		MALMY_ENGINE_API ComponentType getComponentTypeFromHash(u32 hash);
		MALMY_ENGINE_API int getComponentTypesCount();
		MALMY_ENGINE_API const char* getComponentTypeID(int index);
		...
		namespace detail
		{
			...
			template <> MALMY_ENGINE_API Path readFromStream<Path>(InputBlob& stream);
			template <> MALMY_ENGINE_API void writeToStream<Path>(OutputBlob& stream, Path);
			template <> MALMY_ENGINE_API void writeToStream<const Path&>(OutputBlob& stream, const Path& path);
			template <> MALMY_ENGINE_API const char* readFromStream<const char*>(InputBlob& stream);
			template <> MALMY_ENGINE_API void writeToStream<const char*>(OutputBlob& stream, const char* path);
		...
		namespace internal
		{
			static const unsigned int FRONT_SIZE = sizeof("Malmy::Reflection::internal::GetTypeNameHelper<") - 1u;
