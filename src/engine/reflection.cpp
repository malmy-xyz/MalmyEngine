...
{
	ComponentLink* link = MALMY_NEW(*g_allocator, ComponentLink);
...
void init(IAllocator& allocator)
{
	...
	g_enums = MALMY_NEW(allocator, Array<const EnumBase*>)(allocator);
}
...
void shutdown()
{
	...
	MALMY_DELETE(*g_allocator, g_enums);
	g_allocator = nullptr;
}
