...
FileIterator* createFileIterator(const char* path, IAllocator& allocator)
{
	...
	copyString(tmp, path);
	catString(tmp, "/*");
	auto* iter = MALMY_NEW(allocator, FileIterator);
...
void destroyFileIterator(FileIterator* iterator)
{
	...
	MALMY_DELETE(*iterator->allocator, iterator);
}
...
void destroyProcess(Process& process)
{
	...
	MALMY_DELETE(process.allocator, &process);
}


Process* createProcess(const char* cmd, const char* args, IAllocator& allocator)
{
	auto* process = MALMY_NEW(allocator, Process)(allocator);
	...
	{
		MALMY_DELETE(allocator, process);
		return nullptr;
	}
