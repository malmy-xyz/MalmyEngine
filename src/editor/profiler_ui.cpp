...
struct ProfilerUIImpl final : public ProfilerUI
{
		...
		, m_engine(engine)
	{
		...
		m_is_open = false;
		m_is_paused = true;
		m_allocation_root = MALMY_NEW(m_allocator, AllocationStackNode)(nullptr, 0, m_allocator);
		m_filter[0] = 0;
	...
	~ProfilerUIImpl()
	{
		...
		MALMY_DELETE(m_allocator, m_allocation_root);
	...
	size_t size)
{
	...
	auto new_node = MALMY_NEW(m_allocator, AllocationStackNode)(external_node, size, m_allocator);
...
ProfilerUI* ProfilerUI::create(Engine& engine)
{
	...
	return MALMY_NEW(engine.getAllocator(), ProfilerUIImpl)(allocator, engine);
}
...
void ProfilerUI::destroy(ProfilerUI& ui)
{
	...
	MALMY_DELETE(ui_impl.m_engine.getAllocator(), &ui);
}
