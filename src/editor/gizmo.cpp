...
namespace Malmy
{
	...
	struct GizmoImpl MALMY_FINAL : public Gizmo
	{
			...
			, m_immediate_count(0)
		{
			...
			editor.projectDestroyed().bind<GizmoImpl, &GizmoImpl::onProjectDestroyed>(this);
		}


		~GizmoImpl()
		{
			m_editor.projectDestroyed().unbind<GizmoImpl, &GizmoImpl::onProjectDestroyed>(this);
		}
		...
		Matrix getMatrix(GameObject gameobject) const
		{
			...
			{
				mtx = project->getPositionAndRotation(gameobject);
				mtx.translate(mtx.getRotation().rotate(m_offset));
			}
			else if (m_pivot == Pivot::CENTER)
			{
				mtx = project->getPositionAndRotation(gameobject);
				float scale = project->getScale(gameobject);
				Matrix scale_mtx = Matrix::IDGAMEOBJECT;
				scale_mtx.m11 = scale_mtx.m22 = scale_mtx.m33 = scale;
				Vec3 center = m_editor.getRenderInterface()->getModelCenter(gameobject);
			...
			if (m_coord_system == CoordSystem::WORLD)
			{
				...
				mtx = Matrix::IDGAMEOBJECT;
				mtx.setTranslation(pos);
			}
			return mtx;
		}

		static float getScale(const Vec3& camera_pos, float fov, const Vec3& pos, float gameobject_scale, bool is_ortho)
		{
			if (is_ortho) return 2;
			float scale = tanf(fov * 0.5f) * (pos - camera_pos).length() * 2;
			return scale / (10 / gameobject_scale);
		}
			...
			bool is_ortho) const
		{
			...
			Matrix scale_mtx = Matrix::IDGAMEOBJECT;
			auto gameobject_pos = gizmo_mtx.getTranslation();
			float scale = getScale(camera_pos, fov, gameobject_pos, gizmo_mtx.getXVector().length(), is_ortho);
			scale_mtx.m11 = scale_mtx.m22 = scale_mtx.m33 = scale;

			Vec3 to_gameobject_dir = is_ortho ? camera_dir : camera_pos - gameobject_pos;
			...
			if (dotProduct(gizmo_mtx.getXVector(), to_gameobject_dir) < 0) mtx.setXVector(-mtx.getXVector());
			if (dotProduct(gizmo_mtx.getYVector(), to_gameobject_dir) < 0) mtx.setYVector(-mtx.getYVector());
			if (dotProduct(gizmo_mtx.getZVector(), to_gameobject_dir) < 0) mtx.setZVector(-mtx.getZVector());
			...
			if (m_is_dragging)
			{
				...
				Vec2 p = ri->worldToScreenPixels(gameobject_pos);
		...
		void renderArc(const Vec3& pos, const Vec3& n, const Vec3& origin, float angle, u32 color) const
		{
			...
			RenderInterface* ri = m_editor.getRenderInterface();
			ri->render(Matrix::IDGAMEOBJECT, indices, count * 3, vertices, count + 2, false);
		}
			...
			bool is_ortho)
		{
			Axis transform_axis = is_active ? m_transform_axis : Axis::NONE;
			Matrix scale_mtx = Matrix::IDGAMEOBJECT;
			Vec3 gameobject_pos = gizmo_mtx.getTranslation();
			float scale = getScale(camera_pos, fov, gameobject_pos, gizmo_mtx.getXVector().length(), is_ortho);
			scale_mtx.m11 = scale_mtx.m22 = scale_mtx.m33 = scale;
			...
			if (dotProduct(gizmo_mtx.getXVector(), to_gameobject_dir) < 0) right = -right;
			if (dotProduct(gizmo_mtx.getYVector(), to_gameobject_dir) < 0) up = -up;
			if (dotProduct(gizmo_mtx.getZVector(), to_gameobject_dir) < 0) dir = -dir;
			...
			else
			{
				...
				Vec3 origin = (m_start_plane_point - gameobject_pos).normalized();
				renderArc(gameobject_pos, n * scale, origin * scale, m_angle_accum, 0x8800a5ff);
			...
			const Vec3& dir) const
		{
			...
			if (dotProduct(gizmo_mtx.getXVector(), to_gameobject_dir) < 0) x = -x;
			if (dotProduct(gizmo_mtx.getYVector(), to_gameobject_dir) < 0) y = -y;
			if (dotProduct(gizmo_mtx.getZVector(), to_gameobject_dir) < 0) z = -z;
		...
		void collide(const Matrix& gizmo_mtx)
		{
			...
			bool is_ortho = render_interface->isCameraOrtho(edit_camera.gameobject);
			auto camera_pos = m_editor.getProject()->getPosition(edit_camera.gameobject);
			auto camera_dir = m_editor.getProject()->getRotation(edit_camera.gameobject).rotate(Vec3(0, 0, -1));
			float fov = render_interface->getCameraFOV(edit_camera.gameobject);

			Vec3 origin, cursor_dir;
			Vec2 mouse_pos = m_editor.getMousePos();
			m_editor.getRenderInterface()->getRay(edit_camera.gameobject, mouse_pos, origin, cursor_dir);
		...
		Vec3 getMousePlaneIntersection(const Vec2& mouse_pos, const Matrix& gizmo_mtx, Axis transform_axis) const
		{
			...
			Vec3 origin, dir;
			m_editor.getRenderInterface()->getRay(camera.gameobject, mouse_pos, origin, dir);
			dir.normalize();
		...
		void rotate()
		{
			...
			if (m_editor.getSelectedEntities()[0] == m_entities[m_active])
			{
				...
				{
					Vec3 pos = project->getPosition(m_editor.getSelectedEntities()[i]);
					Quat old_rot = project->getRotation(m_editor.getSelectedEntities()[i]);
			...
			{
				Quat old_rot = project->getRotation(m_entities[m_active]);
				Quat new_rot = Quat(axis, angle) * old_rot;
				new_rot.normalize();
				new_rotations.push(new_rot);
		...
		void scale()
		{
			...
			Vec3 gameobject_to_intersection = intersection - gizmo_mtx.getTranslation();
			if (dotProduct(delta_vec, gameobject_to_intersection) < 0) delta = -delta;
			if (!m_is_step || delta > float(getStep()))
			{
				...
				if (m_entities[m_active] == m_editor.getSelectedEntities()[0])
				{
					...
					{
						float scale = m_editor.getProject()->getScale(m_editor.getSelectedEntities()[i]);
						scale += delta;
				...
				{
					float scale = m_editor.getProject()->getScale(m_entities[m_active]);
					scale += delta;
					new_scales.push(scale);
		...
		void translate()
		{
			...
			if (!m_is_step || delta.length() > float(getStep()))
			{
				...
				if (m_entities[m_active] == m_editor.getSelectedEntities()[0])
				{
					for (int i = 0, ci = m_editor.getSelectedEntities().size(); i < ci; ++i)
					{
						Vec3 pos = m_editor.getProject()->getPosition(m_editor.getSelectedEntities()[i]);
						pos += delta;
						new_positions.push(pos);
					}
				...
				{
					Vec3 pos = m_editor.getProject()->getPosition(m_entities[m_active]);
					pos += delta;
					new_positions.push(pos);
		...
		void render() override
		{
			...
			//tus kontrolu burda yapiliyor
			//bastiysa moda gore islem yapiyor
			transform();
		...
		bool m_is_step;
		int m_count;
		GameObject m_entities[MAX_GIZMOS];
...
{
	return MALMY_NEW(editor.getAllocator(), GizmoImpl)(editor);
}


void Gizmo::destroy(Gizmo& gizmo)
{
	MALMY_DELETE(static_cast<GizmoImpl&>(gizmo).m_editor.getAllocator(), &gizmo);
}
