...
struct GizmoImpl MALMY_FINAL : public Gizmo
{
	...
	Matrix getMatrix(Entity entity) const
	{
		...
		{
			mtx = project->getPositionAndRotation(entity);
			mtx.translate(mtx.getRotation().rotate(m_offset));
		}
		else if (m_pivot == Pivot::CENTER)
		{
			mtx = project->getPositionAndRotation(entity);
			float scale = project->getScale(entity);
	...
	void collide(const Matrix& gizmo_mtx)
	{
		...
		auto camera_pos = m_editor.getProject()->getPosition(edit_camera.entity);
		auto camera_dir = m_editor.getProject()->getRotation(edit_camera.entity).rotate(Vec3(0, 0, -1));
