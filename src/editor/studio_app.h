...
#define MALMY_STUDIO_ENTRY(plugin_name) \
	extern "C" MALMY_LIBRARY_EXPORT StudioApp::IPlugin* setStudioApp(StudioApp& app)
...
namespace Malmy
{
	...
	class MALMY_EDITOR_API StudioApp
	{
	public:
		struct IPlugin
		{
		...
		struct IAddComponentPlugin
		{
			...
			virtual void onGUI(bool create_gameobject, bool from_filter) = 0;
		...
		struct MALMY_EDITOR_API StaticPluginRegister
		{
