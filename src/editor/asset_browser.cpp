	...
	, m_registered_extensions(app.getWorldEditor().getAllocator())
{
	...
	StaticString<MAX_PATH_LENGTH> path(base_path, ".malmy");
	...
	m_auto_reload_action = MALMY_NEW(allocator, Action)("Auto-reload", "Auto-reload assets", "autoReload");
	...
	m_back_action = MALMY_NEW(allocator, Action)("Back", "Back in asset history", "back");
	m_back_action->is_global = false;
	m_back_action->func.bind<AssetBrowser, &AssetBrowser::goBack>(this);
	m_forward_action = MALMY_NEW(allocator, Action)("Forward", "Forward in asset history", "forward");
	m_forward_action->is_global = false;
	m_forward_action->func.bind<AssetBrowser, &AssetBrowser::goForward>(this);
	m_refresh_action = MALMY_NEW(allocator, Action)("Refresh", "Refresh assets", "refresh");
...
void AssetBrowser::thumbnail(FileInfo& tile)
{
	...
	else
	{
		...
		StaticString<MAX_PATH_LENGTH> path(".malmy/asset_tiles/", tile.file_path_hash, ".dds");
