...
struct FileSystemWatcherTask MALMY_FINAL : public MT::Task
{
...
class FileSystemWatcherPC MALMY_FINAL : public FileSystemWatcher
{
public:
	...
	virtual ~FileSystemWatcherPC()
	{
		if (m_task)
		{
			...
			m_task->destroy();
			MALMY_DELETE(m_allocator, m_task);
		}
	}
	...
	{
		m_task = MALMY_NEW(m_allocator, FileSystemWatcherTask)(path, *this, m_allocator);
...
{
	FileSystemWatcherPC* watcher = MALMY_NEW(allocator, FileSystemWatcherPC)(allocator);
	if (!watcher->start(path))
	{
		MALMY_DELETE(allocator, watcher);
		return nullptr;
	}
	return watcher;
}
...
void FileSystemWatcher::destroy(FileSystemWatcher* watcher)
{
	...
	MALMY_DELETE(pc_watcher->m_allocator, pc_watcher);
}
