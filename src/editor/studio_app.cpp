...
public:
		...
		, m_events(m_allocator)
	{
		...
		m_window = SDL_CreateWindow("Malmy Engine", 0, 0, 800, 600, SDL_WINDOW_SHOWN | SDL_WINDOW_RESIZABLE | SDL_WINDOW_ALLOW_HIGHDPI);
	...
	void showHierarchy(Entity entity, const Array<Entity>& selected_entities)
	{
		...
		bool has_child = project->getFirstChild(entity).isValid();
		...
		if (ImGui::BeginPopup("entity_context_menu"))
		{
			if (ImGui::MenuItem("Create child"))
			{
				...
				Vec3 pos = m_editor->getProject()->getPosition(entity);
	...
	void onEntityListGUI()
	{
		...
		if (ImGui::BeginDock("Entity List", &m_is_entity_list_open))
		{
			...
			if (ImGui::BeginChild("entities"))
			{
				...
				if (filter[0] == '\0')
				{
					for (Entity e = project->getFirstEntity(); e.isValid(); e = project->getNextEntity(e))
					{
