...
namespace Malmy
{
	...
	struct LuaPlugin : public StudioApp::GUIPlugin
	{
			...
			: editor(app.getWorldEditor())
		{
			...
			Action* action = MALMY_NEW(editor.getAllocator(), Action)(name, name, name);
	...
	class StudioAppImpl MALMY_FINAL : public StudioApp
	{
	public:
		StudioAppImpl()
			: m_is_gameobject_list_open(true)
			...
			, m_events(m_allocator)
		{
			...
			m_window = SDL_CreateWindow("Malmy Engine", 0, 0, 1920, 1080, SDL_WINDOW_SHOWN | SDL_WINDOW_RESIZABLE | SDL_WINDOW_ALLOW_HIGHDPI);
			...
			m_asset_browser = MALMY_NEW(m_allocator, AssetBrowser)(*this);
			m_property_grid = MALMY_NEW(m_allocator, PropertyGrid)(*this);
			m_profiler_ui = ProfilerUI::create(*m_engine);
			m_log_ui = MALMY_NEW(m_allocator, LogUI)(m_editor->getAllocator());
			...
			m_custom_pivot_action = MALMY_NEW(m_editor->getAllocator(), Action)(
		...
		~StudioAppImpl()
		{
			...
			{
				MALMY_DELETE(m_editor->getAllocator(), i);
			}
			m_plugins.clear();
			...
			{
				MALMY_DELETE(m_editor->getAllocator(), a);
			}
			m_actions.clear();

			ProfilerUI::destroy(*m_profiler_ui);
			MALMY_DELETE(m_allocator, m_asset_browser);
			MALMY_DELETE(m_allocator, m_property_grid);
			MALMY_DELETE(m_allocator, m_log_ui);
		...
		void addPlugin(IAddComponentPlugin& plugin)
		{
			...
			auto* node = MALMY_NEW(m_allocator, AddCmpTreeNode);
		...
		void insertAddCmpNode(AddCmpTreeNode& parent, AddCmpTreeNode* node)
		{
			...
			auto* new_group = MALMY_NEW(m_allocator, AddCmpTreeNode);
		...
		{
			struct Plugin MALMY_FINAL : public IAddComponentPlugin
			{
			...
			auto* plugin = MALMY_NEW(allocator, Plugin);
		...
		void update()
		{
			...
			m_editor->update();
			m_engine->update(*m_editor->getProject());
		...
		void showWelcomeScreen()
		{
			...
			{
				ImGui::Text("Welcome to Malmy Engine");
				...
				{
					if (ImGui::Button("New Project")) m_is_welcome_screen_open = false;

					ImGui::Separator();
					ImGui::Text("Open Project:");
					ImGui::Indent();
					for (auto& univ : m_projects)
					{
						if (ImGui::MenuItem(univ.data))
						{
							m_editor->loadProject(univ.data);
							setTitle(univ.data);
		...
		void save()
		{
			...
			if (m_editor->getProject()->getName()[0])
			{
				m_editor->saveProject(m_editor->getProject()->getName(), true);
			}
			else
			{
				saveAs();
		...
		void onSaveAsDialogGUI()
		{
			...
			if (ImGui::Begin("Save Project As", &m_is_save_as_dialog_open))
			{
				static char name[64] = "";
				...
				if (ImGui::Button("Save"))
				{
					m_is_save_as_dialog_open = false;
					setTitle(name);
					m_editor->saveProject(name, true);
					scanProjects();
				}
				ImGui::SameLine();
		...
		void addGameObject() { m_editor->addGameObject(); }
		...
		void toggleGameObjectList() { m_is_gameobject_list_open = !m_is_gameobject_list_open; }
		bool isGameObjectListOpen() const { return m_is_gameobject_list_open; }
		...
		void unparent()
		{
			...
			m_editor->makeParent(INVALID_GAMEOBJECT, entities[0]);
		}
		...
		void destroySelectedGameObject()
		{
		...
		{
			auto* a = MALMY_NEW(m_editor->getAllocator(), Action)(label_short, label_long, name);
		...
		{
			auto* a = MALMY_NEW(m_editor->getAllocator(), Action)(
		...
		void onCreateGameObjectWithComponentGUI()
		{
			doMenuItem(*getAction("createGameObject"), true);
			ImGui::Separator();
		...
		void gameobjectMenu()
		{
			...
			bool is_any_gameobject_selected = !selected_entities.empty();
			if (ImGui::BeginMenu("Create"))
			{
				onCreateGameObjectWithComponentGUI();
				ImGui::EndMenu();
			}
			doMenuItem(*getAction("destroyGameObject"), is_any_gameobject_selected);
			doMenuItem(*getAction("savePrefab"), selected_entities.size() == 1);
			doMenuItem(*getAction("makeParent"), selected_entities.size() == 2);
			bool can_unparent = selected_entities.size() == 1 && m_editor->getProject()->getParent(selected_entities[0]).isValid();
		...
		void editMenu()
		{
			...
			bool is_any_gameobject_selected = !m_editor->getSelectedEntities().empty();
			//doMenuItem(*getAction("undo"), m_editor->canUndo());
			//doMenuItem(*getAction("redo"), m_editor->canRedo());
			//ImGui::Separator();
			doMenuItem(*getAction("copy"), is_any_gameobject_selected);
			doMenuItem(*getAction("paste"), m_editor->canPasteEntities());
			doMenuItem(*getAction("duplicate"), is_any_gameobject_selected);
			ImGui::Separator();
			//doMenuItem(*getAction("orbitCamera"), is_any_gameobject_selected || m_editor->isOrbitCamera());
		...
		void fileMenu()
		{
			...
			doMenuItem(*getAction("newProject"), true);
			if (ImGui::BeginMenu("Open"))
			{
				...
				for (auto& univ : m_projects)
				{
						...
						ImGui::MenuItem(univ.data))
					{
						if (m_editor->isProjectChanged())
						{
							copyString(m_project_to_load, univ.data);
							m_confirm_load = true;
						}
						else
						{
		...
		void toolsMenu()
		{
			...
			bool is_any_gameobject_selected = !m_editor->getSelectedEntities().empty();
			doMenuItem(*getAction("lookAtSelected"), is_any_gameobject_selected);
			doMenuItem(*getAction("toggleGameMode"), true);
			doMenuItem(*getAction("toggleMeasure"), true);
			doMenuItem(*getAction("snapDown"), is_any_gameobject_selected);
		...
		void viewMenu()
		{
			...
			doMenuItem(*getAction("gameobjectList"), true);
		...
		float showMainMenu()
		{
			...
			if(ImGui::BeginPopupModal("Confirm"))
			{
				...
				if (ImGui::Button("Continue"))
				{
					m_editor->loadProject(m_project_to_load);
					setTitle(m_project_to_load);
					ImGui::CloseCurrentPopup();
				}
				ImGui::SameLine();
				if (ImGui::Button("Cancel")) ImGui::CloseCurrentPopup();
			...
			if (ImGui::BeginMainMenuBar())
			{
				...
				toolsMenu();
				viewMenu();
				//buraya help menu falanda yapilcak
		...
		void showHierarchy(GameObject gameobject, const Array<GameObject>& selected_entities)
		{
			char buffer[1024];
			Project* project = m_editor->getProject();
			getGameObjectListDisplayName(*m_editor, buffer, sizeof(buffer), gameobject);
			bool selected = selected_entities.indexOf(gameobject) >= 0;
			ImGui::PushID(gameobject.index);
			ImGuiTreeNodeFlags flags = ImGuiTreeNodeFlags_OpenOnArrow | ImGuiTreeNodeFlags_AllowItemOverlap;
			bool has_child = project->getFirstChild(gameobject).isValid();
			if (!has_child) flags = ImGuiTreeNodeFlags_Leaf;
			if (selected) flags |= ImGuiTreeNodeFlags_Selected;
			bool node_open = ImGui::TreeNodeEx(buffer, flags);
			if (ImGui::IsItemClicked(0)) m_editor->selectEntities(&gameobject, 1, true);
			if (ImGui::IsMouseReleased(1) && ImGui::IsItemHovered()) ImGui::OpenPopup("gameobject_context_menu");
			if (ImGui::BeginPopup("gameobject_context_menu"))
			{
				if (ImGui::MenuItem("Create child"))
				{
					m_editor->beginCommandGroup(crc32("create_child_gameobject"));
					GameObject child = m_editor->addGameObject();
					m_editor->makeParent(gameobject, child);
					Vec3 pos = m_editor->getProject()->getPosition(gameobject);
			...
			if (ImGui::BeginDragDropTarget())
			{
				...
				{
					GameObject dropped_gameobject = *(GameObject*)payload->Data;
					if (dropped_gameobject != gameobject)
					{
						m_editor->makeParent(gameobject, dropped_gameobject);
		...
		void onGameObjectListGUI()
		{
			PROFILE_FUNCTION();
			const Array<GameObject>& entities = m_editor->getSelectedEntities();
			static char filter[64] = "";
			if (ImGui::BeginDock("GameObject List", &m_is_gameobject_list_open))
			{
				...
				if (ImGui::BeginChild("entities"))
				{
					...
					if (filter[0] == '\0')
					{
						for (GameObject e = project->getFirstGameObject(); e.isValid(); e = project->getNextGameObject(e))
						{
							if (!project->getParent(e).isValid())
							{
					...
					else
					{
						...
						{
							char buffer[1024];
							getGameObjectListDisplayName(*m_editor, buffer, sizeof(buffer), e);
							...
							{
								ImGui::Text("%s", buffer);
								ImGui::SetDragDropPayload("gameobject", &e, sizeof(e));
								ImGui::EndDragDropSource();
							}
							ImGui::PopID();
						}
				...
				/*if (ImGui::BeginDragDropTarget())
				{
					...
					{
						GameObject dropped_gameobject = *(GameObject*)payload->Data;
						m_editor->makeParent(INVALID_GAMEOBJECT, dropped_gameobject);
					}
					ImGui::EndDragDropTarget();
				}*/
			}
		...
		void saveSettings()
		{
			...
			m_settings.m_is_gameobject_list_open = m_is_gameobject_list_open;
		...
		void loadSettings()
		{
			...
			m_is_gameobject_list_open = m_settings.m_is_gameobject_list_open;
		...
		{
			//file klasoru alti
			addAction<&StudioAppImpl::newProject>("New", "New project", "newProject");
			addAction<&StudioAppImpl::save>("Save", "Save project", "save", SDLK_LCTRL, 'S', -1);
			addAction<&StudioAppImpl::saveAs>("Save As", "Save project as", "saveAs", SDLK_LCTRL, SDLK_LSHIFT, 'S');
			...
			addAction<&StudioAppImpl::copy>("Copy", "Copy gameobject", "copy", SDLK_LCTRL, 'C', -1);
			addAction<&StudioAppImpl::paste>("Paste", "Paste gameobject", "paste", SDLK_LCTRL, 'V', -1);
			addAction<&StudioAppImpl::duplicate>("Duplicate", "Duplicate gameobject", "duplicate", SDLK_LCTRL, 'D', -1);
			...
			//tool lar
			addAction<&StudioAppImpl::addGameObject>("Create empty", "Create empty gameobject", "createGameObject");
			addAction<&StudioAppImpl::destroySelectedGameObject>("Destroy", "Destroy gameobject", "destroyGameObject", SDLK_DELETE, -1, -1);
			addAction<&StudioAppImpl::savePrefab>("Save prefab", "Save selected entities as prefab", "savePrefab");
			addAction<&StudioAppImpl::makeParent>("Make parent", "Make gameobject parent", "makeParent");
			addAction<&StudioAppImpl::unparent>("Unparent", "Unparent gameobject", "unparent");
			...
			addAction<&StudioAppImpl::lookAtSelected>("Look at selected", "Look at selected gameobject", "lookAtSelected");
			...
			addAction<&StudioAppImpl::toggleGameObjectList>("GameObject List", "Toggle gameobject list", "gameobjectList")
				.is_selected.bind<StudioAppImpl, &StudioAppImpl::isGameObjectListOpen>(this);
		...
		void onPluginChanged(const char* path)
		{
			...
			if (!PathUtils::hasExtension(path, ext) && !PathUtils::hasExtension(path, "pda")) 
				return;
		...
		void tryReloadPlugin()
		{
			...
			Project* project = m_editor->getProject();
			for (IScene* scene : project->getScenes())
			{
			...
			m_watched_plugin.plugin->createScenes(*project);
			for (IScene* scene : project->getScenes())
			{
				if (&scene->getPlugin() != m_watched_plugin.plugin) continue;
		...
		void selectGameObject(GameObject e)
		{
		...
		{
			return m_editor->addGameObject();
		}

		void createComponent(GameObject e, int type)
		{
			m_editor->selectEntities(&e, 1, false);
		...
		struct SetPropertyVisitor : public Reflection::IPropertyVisitor
		{
			...
			{ 
				...
				editor->setProperty(cmp.type, 0, prop, &cmp.gameobject, 1, &val, sizeof(val));
			}


			// TODO 
			void visit(const Reflection::Property<float>& prop) override { notSupported(prop); }
			void visit(const Reflection::Property<GameObject>& prop) override { notSupported(prop); }
			...
			void visit(const Reflection::Property<const char*>& prop) override
			{
				...
				editor->setProperty(cmp.type, 0, prop, &cmp.gameobject, 1, str, stringLength(str) + 1);
			}
		...
		static int createGameObjectEx(lua_State* L)
		{
			auto* studio = LuaWrapper::checkArg<StudioAppImpl*>(L, 1);
			LuaWrapper::checkTableArg(L, 2);

			WorldEditor& editor = *studio->m_editor;
			editor.beginCommandGroup(crc32("createGameObjectEx"));
			GameObject e = editor.addGameObject();
			...
			while (lua_next(L, -2) != 0)
			{
				...
				else
				{
					...
					IScene* scene = editor.getProject()->getScene(cmp_type);
					if (scene)
					{
		...
		void saveProjectAs(const char* basename, bool save_path)
		{
			m_editor->saveProject(basename, save_path);
		}
		...
		void createLua()
		{
			...
			REGISTER_FUNCTION(selectGameObject);
			REGISTER_FUNCTION(createGameObject);
			REGISTER_FUNCTION(createComponent);
			REGISTER_FUNCTION(destroyGameObject);
			REGISTER_FUNCTION(newProject);
			REGISTER_FUNCTION(saveProject);
			REGISTER_FUNCTION(saveProjectAs);
			...
			LuaWrapper::createSystemFunction(L, "Editor", "createGameObjectEx", &createGameObjectEx);
		}
		...
		void packDataScanResources(AssociativeArray<u32, PackFileInfo>& infos)
		{
			...
			StaticString<MAX_PATH_LENGTH> mproject_path;
			mproject_path << "projects/" << m_editor->getProject()->getName() << "/";
			packDataScan(mproject_path, infos);
			mproject_path.data[0] = 0;
			mproject_path << "projects/" << m_editor->getProject()->getName() << ".mproject";
			u32 hash = crc32(mproject_path);
			auto& out_info = infos.emplace(hash);
			copyString(out_info.path, MAX_PATH_LENGTH, mproject_path);
			out_info.hash = hash;
			out_info.size = PlatformInterface::getFileSize(mproject_path);
			out_info.offset = ~0UL;
		...
		void onPackDataGUI()
		{
			if (ImGui::BeginDock("Pack data", &m_is_pack_data_dialog_open))
			{
				...
				ImGui::Combo("Mode", (int*)&m_pack.mode, "All files\0Loaded project\0");
		...
		void loadLuaPlugin(const char* dir, const char* filename)
		{
			...
			if (file.open(path, FS::Mode::OPEN_AND_READ))
			{
				...
				LuaPlugin* plugin = MALMY_NEW(m_editor->getAllocator(), LuaPlugin)(*this, src, filename);
				addPlugin(*plugin);
		...
		{
			m_projects.clear();
			auto* iter = PlatformInterface::createFileIterator("projects/", m_allocator);
			PlatformInterface::FileInfo info;
			while (PlatformInterface::getNextFile(iter, &info))
			{
		...
		{
			if (!PlatformInterface::fileExists("../MalmyStudio.lnk")) return;
		...
		Array<StaticString<MAX_PATH_LENGTH>> m_projects;
		...
		bool m_confirm_load;
		bool m_confirm_new;
		char m_project_to_load[MAX_PATH_LENGTH];
