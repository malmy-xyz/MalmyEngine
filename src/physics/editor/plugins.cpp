...
namespace
{
	...
	struct GizmoPlugin MALMY_FINAL : public WorldEditor::Plugin
	{
		...
		static void showSphericalJointGizmo(ComponentUID cmp)
		{
			...
			Project& project = phy_scene->getProject();
			auto* render_scene = static_cast<RenderScene*>(project.getScene(RENDERER_HASH));
			if (!render_scene) return;

			GameObject other_gameobject = phy_scene->getJointConnectedBody(cmp.gameobject);
			if (!other_gameobject.isValid()) return;


			RigidTransform local_frame0 = phy_scene->getJointLocalFrame(cmp.gameobject);
			RigidTransform global_frame0 = project.getTransform(cmp.gameobject).getRigidPart() * local_frame0;
			Vec3 joint_pos = global_frame0.pos;
			Matrix mtx0 = global_frame0.toMatrix();
			...
			RigidTransform local_frame1 = phy_scene->getJointConnectedBodyLocalFrame(cmp.gameobject);
			RigidTransform global_frame1 = project.getTransform(other_gameobject).getRigidPart() * local_frame1;
			Matrix mtx1 = global_frame1.toMatrix();

			bool use_limit = phy_scene->getSphericalJointUseLimit(cmp.gameobject);
			if (use_limit)
			{
				Vec2 limit = phy_scene->getSphericalJointLimit(cmp.gameobject);
				Vec3 other_pos = project.getPosition(other_gameobject);
		...
		static void showDistanceJointGizmo(ComponentUID cmp)
		{
			...
			GameObject other_gameobject = phy_scene->getJointConnectedBody(cmp.gameobject);
			if (!other_gameobject.isValid()) return;
			RigidTransform local_frame = phy_scene->getJointConnectedBodyLocalFrame(cmp.gameobject);

			Vec3 pos = project.getPosition(other_gameobject);
			Vec3 other_pos = (project.getTransform(other_gameobject).getRigidPart() * local_frame).pos;
			...
			Vec3 force = phy_scene->getDistanceJointLinearForce(cmp.gameobject);
		...
		{
			auto* phy_scene = static_cast<PhysicsScene*>(cmp.scene);
			GameObject connected_body = phy_scene->getJointConnectedBody(cmp.gameobject);
			Vec2 limit = phy_scene->getHingeJointLimit(cmp.gameobject);
			bool use_limit = phy_scene->getHingeJointUseLimit(cmp.gameobject);
			if (!connected_body.isValid()) return;
			RigidTransform global_frame1 = phy_scene->getJointConnectedBodyLocalFrame(cmp.gameobject);
			global_frame1 = phy_scene->getProject().getTransform(connected_body).getRigidPart() * global_frame1;
		...
		static void showBoxActorGizmo(ComponentUID cmp, RenderScene& render_scene)
		{
			...
			Vec3 extents = phy_scene->getHalfExtents(cmp.gameobject);
			Matrix mtx = project.getPositionAndRotation(cmp.gameobject);
		...
		static void showSphereActorGizmo(ComponentUID cmp, RenderScene& render_scene)
		{
			...
			float radius = phy_scene->getSphereRadius(cmp.gameobject);
			Vec3 pos = project.getPosition(cmp.gameobject);
		...
		static void showCapsuleActorGizmo(ComponentUID cmp, RenderScene& render_scene)
		{
			...
			float radius = phy_scene->getCapsuleRadius(cmp.gameobject);
			float height = phy_scene->getCapsuleHeight(cmp.gameobject);
			Matrix mtx = project.getPositionAndRotation(cmp.gameobject);
		...
		bool showGizmo(ComponentUID cmp) override
		{
			...
			{
				float height = phy_scene->getControllerHeight(cmp.gameobject);
				float radius = phy_scene->getControllerRadius(cmp.gameobject);
			...
			{
				physx::PxD6Joint* joint = static_cast<physx::PxD6Joint*>(phy_scene->getJoint(cmp.gameobject));
				showD6JointGizmo(project.getTransform(cmp.gameobject).getRigidPart(), *render_scene, joint);
				return true;
			}
	...
	struct PhysicsUIPlugin MALMY_FINAL : public StudioApp::GUIPlugin
	{
		...
		{
			Action* action = MALMY_NEW(m_editor.getAllocator(), Action)("Physics", "Toggle physics UI", "physics");
		...
		void onLayersGUI()
		{
			auto* scene = static_cast<PhysicsScene*>(m_editor.getProject()->getScene(crc32("physics")));
		...
		{
			auto* scene = static_cast<PhysicsScene*>(m_editor.getProject()->getScene(crc32("physics")));
			auto* render_scene = static_cast<RenderScene*>(m_editor.getProject()->getScene(RENDERER_HASH));
			if (!render_scene) return;
			...
			if (count > 0 && ImGui::CollapsingHeader("Joints"))
			{
				...
				{
					ComponentUID cmp;
					cmp.gameobject = scene->getJointGameObject(i);
					cmp.scene = scene;
					physx::PxJoint* joint = scene->getJoint(cmp.gameobject);
					switch ((physx::PxJointConcreteType::Enum)scene->getJoint(cmp.gameobject)->getConcreteType())
					{
					...
					case physx::PxJointConcreteType::eD6:
						...
						GizmoPlugin::showD6JointGizmo(m_editor.getProject()->getTransform(cmp.gameobject).getRigidPart(),
							*render_scene,
					...
					ImGui::PushID(i);
					char tmp[256];
					getGameObjectListDisplayName(m_editor, tmp, lengthOf(tmp), cmp.gameobject);
					bool b = false;
					if (ImGui::Selectable(tmp, &b)) m_editor.selectEntities(&cmp.gameobject, 1, false);
					ImGui::NextColumn();

					GameObject other_gameobject = scene->getJointConnectedBody(cmp.gameobject);
					getGameObjectListDisplayName(m_editor, tmp, lengthOf(tmp), other_gameobject);
					if (other_gameobject.isValid() && ImGui::Selectable(tmp, &b)) m_editor.selectEntities(&other_gameobject, 1, false);
					ImGui::NextColumn();
					ImGui::PopID();
				}
				ImGui::Columns();
		...
		{
			auto* scene = static_cast<PhysicsScene*>(m_editor.getProject()->getScene(crc32("physics")));
			Vec3 camera_pos = m_editor.getProject()->getPosition(m_editor.getEditCamera().gameobject);
		...
		void onActorGUI()
		{
			...
			{
				ComponentUID cmp;
				cmp.gameobject = scene->getActorGameObject(i);
				if (!cmp.gameobject.isValid()) continue;
				ImGui::PushID(i);
				char tmp[255];
				getGameObjectListDisplayName(m_editor, tmp, lengthOf(tmp), cmp.gameobject);
				bool selected = false;
				if (ImGui::Selectable(tmp, &selected)) m_editor.selectEntities(&cmp.gameobject, 1, false);
				ImGui::NextColumn();
		...
		void autogeneratePhySkeleton(PhysicsScene& scene, GameObject gameobject, Model* model)
		{
			while (scene.getRagdollRootBone(gameobject))
			{
				scene.destroyRagdollBone(gameobject, scene.getRagdollRootBone(gameobject));
			}

			for (int i = 0; i < model->getBoneCount(); ++i)
			{
				auto& bone = model->getBone(i);
				scene.createRagdollBone(gameobject, crc32(bone.name.c_str()));
			}
		}
		...
		void onRagdollGUI()
		{
			...
			{
				ImGui::Text("%s", "Please select an gameobject.");
				return;
			}
			...
			GameObject gameobject = m_editor.getSelectedEntities()[0];
			bool has_model_instance = render_scene->getProject().hasComponent(gameobject, MODEL_INSTANCE_TYPE);
			auto* phy_scene = static_cast<PhysicsScene*>(m_editor.getProject()->getScene(crc32("physics")));

			bool has_ragdoll = render_scene->getProject().hasComponent(gameobject, RAGDOLL_TYPE);
			if (!has_ragdoll || !has_model_instance)
			{
				ImGui::Text("%s", "Please select an gameobject with ragdoll and mesh components.");
				return;
			}

			Matrix mtx = m_editor.getProject()->getMatrix(gameobject);
			...
			if (m_selected_bone >= 0 && m_selected_bone < model->getBoneCount())
			{
				...
				selected_bone = phy_scene->getRagdollBoneByName(gameobject, hash);
			}
			if (visualize) renderBone(*render_scene, *phy_scene, phy_scene->getRagdollRootBone(gameobject), selected_bone);
			ImGui::SameLine();
			if (ImGui::Button("Autogenerate")) autogeneratePhySkeleton(*phy_scene, gameobject, model);
			ImGui::SameLine();
			auto* root = phy_scene->getRagdollRootBone(gameobject);
			...
			if (ImGui::BeginChild("properties", ImVec2(ImGui::GetContentRegionAvailWidth(), 0)))
			{
				...
				else
				{
					...
					onBonePropertiesGUI(*phy_scene, gameobject, crc32(bone.name.c_str()));
				}
			}
			ImGui::EndChild();
		}
		...
		void onBonePropertiesGUI(PhysicsScene& scene, GameObject gameobject, u32 bone_name_hash)
		{
			auto* bone_handle = scene.getRagdollBoneByName(gameobject, bone_name_hash);
			if (!bone_handle)
			{
				if (ImGui::Button("Add"))
				{
					scene.createRagdollBone(gameobject, bone_name_hash);
				}
				return;
			}
			...
			{
				scene.destroyRagdollBone(gameobject, bone_handle);
				return;
			}
	...
	struct PhysicsGeometryPlugin MALMY_FINAL : public AssetBrowser::IPlugin
	{
	...
	struct StudioAppPlugin : StudioApp::IPlugin
	{
			...
			: m_app(app)
		{
			...
			m_ui_plugin = MALMY_NEW(allocator, PhysicsUIPlugin)(app);
			m_gizmo_plugin = MALMY_NEW(allocator, GizmoPlugin)(editor);
			m_geom_plugin = MALMY_NEW(allocator, PhysicsGeometryPlugin)(app);
		...
		~StudioAppPlugin()
		{
			...
			MALMY_DELETE(allocator, m_ui_plugin);
			MALMY_DELETE(allocator, m_gizmo_plugin);
			MALMY_DELETE(allocator, m_geom_plugin);
		}
...
MALMY_STUDIO_ENTRY(physics)
{
	IAllocator& allocator = app.getWorldEditor().getAllocator();
	return MALMY_NEW(allocator, StudioAppPlugin)(app);
}
