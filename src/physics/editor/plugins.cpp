...
struct GizmoPlugin MALMY_FINAL : public WorldEditor::Plugin
{
	...
	static void showSphericalJointGizmo(ComponentUID cmp)
	{
		...
		RigidTransform global_frame0 = project.getTransform(cmp.entity).getRigidPart() * local_frame0;
		Vec3 joint_pos = global_frame0.pos;
		Matrix mtx0 = global_frame0.toMatrix();
		...
		RigidTransform global_frame1 = project.getTransform(other_entity).getRigidPart() * local_frame1;
		...
		if (use_limit)
		{
			...
			Vec3 other_pos = project.getPosition(other_entity);
	...
	static void showDistanceJointGizmo(ComponentUID cmp)
	{
		...
		Vec3 pos = project.getPosition(other_entity);
		Vec3 other_pos = (project.getTransform(other_entity).getRigidPart() * local_frame).pos;
	...
	static void showBoxActorGizmo(ComponentUID cmp, RenderScene& render_scene)
	{
		...
		Matrix mtx = project.getPositionAndRotation(cmp.entity);
	...
	static void showSphereActorGizmo(ComponentUID cmp, RenderScene& render_scene)
	{
		...
		Vec3 pos = project.getPosition(cmp.entity);
	...
	bool showGizmo(ComponentUID cmp) override
	{
		...
		if (cmp.type == D6_JOINT_TYPE)
		{
			...
			showD6JointGizmo(project.getTransform(cmp.entity).getRigidPart(), *render_scene, joint);
			return true;
		}
...
struct PhysicsUIPlugin MALMY_FINAL : public StudioApp::GUIPlugin
{
			...
			for (int i = 0; i < count; ++i)
			{
				...
				switch ((physx::PxJointConcreteType::Enum)scene->getJoint(cmp.entity)->getConcreteType())
				{
					...
					case physx::PxJointConcreteType::eD6:
						...
						GizmoPlugin::showD6JointGizmo(m_editor.getProject()->getTransform(cmp.entity).getRigidPart(),
							*render_scene,
	...
	{
		auto* scene = static_cast<PhysicsScene*>(m_editor.getProject()->getScene(crc32("physics")));
		Vec3 camera_pos = m_editor.getProject()->getPosition(m_editor.getEditCamera().entity);
	...
	void onRagdollGUI()
	{
		...
		bool has_model_instance = render_scene->getProject().hasComponent(entity, MODEL_INSTANCE_TYPE);
		auto* phy_scene = static_cast<PhysicsScene*>(m_editor.getProject()->getScene(crc32("physics")));

		bool has_ragdoll = render_scene->getProject().hasComponent(entity, RAGDOLL_TYPE);
		...
		Matrix mtx = m_editor.getProject()->getMatrix(entity);
