...
FontManager::~FontManager()
{
	if (m_atlas_texture)
	{
		...
		MALMY_DELETE(m_allocator, m_atlas_texture);
	}
}
...
void FontManager::updateFontTexture()
{
	...
	else
	{
		...
		m_atlas_texture = MALMY_NEW(m_allocator, Texture)(Path("draw2d_atlas"), texture_manager, m_allocator);
	}
...
{
	return MALMY_NEW(m_allocator, FontResource)(path, *this, m_allocator);
}


void FontManager::destroyResource(Resource& resource)
{
	MALMY_DELETE(m_allocator, static_cast<FontResource*>(&resource));
}
