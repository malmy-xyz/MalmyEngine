...
struct ColorConstNode : public ShaderEditor::Node
{
	...
	void onGUI() override { ImGui::ColorPicker4("", m_color); }
...
void ShaderEditor::createConnection(Node* node, int pin_index, bool is_input)
{
	...
	if (is_input)
	{
		execute(MALMY_NEW(m_allocator, CreateConnectionCommand)(
...
ShaderEditor::Node* ShaderEditor::createNode(int type)
{
	...
	{
		case NodeType::FRAGMENT_OUTPUT:				return MALMY_NEW(m_allocator, FragmentOutputNode)(*this);
		case NodeType::VERTEX_OUTPUT:				return MALMY_NEW(m_allocator, VertexOutputNode)(*this);
		case NodeType::FRAGMENT_INPUT:				return MALMY_NEW(m_allocator, FragmentInputNode)(*this);
		case NodeType::POSITION_OUTPUT:				return MALMY_NEW(m_allocator, PositionOutputNode)(*this);
		case NodeType::VERTEX_INPUT:				return MALMY_NEW(m_allocator, VertexInputNode)(*this);
		case NodeType::COLOR_CONST:					return MALMY_NEW(m_allocator, ColorConstNode)(*this);
		case NodeType::FLOAT_CONST:					return MALMY_NEW(m_allocator, FloatConstNode)(*this);
		case NodeType::MIX:							return MALMY_NEW(m_allocator, MixNode)(*this);
		case NodeType::SAMPLE:						return MALMY_NEW(m_allocator, SampleNode)(*this);
		case NodeType::UNIFORM:						return MALMY_NEW(m_allocator, UniformNode)(*this);
		case NodeType::VEC4_MERGE:					return MALMY_NEW(m_allocator, Vec4MergeNode)(*this);
		case NodeType::OPERATOR:					return MALMY_NEW(m_allocator, OperatorNode)(*this);
		case NodeType::BUILTIN_UNIFORM:				return MALMY_NEW(m_allocator, BuiltinUniformNode)(*this);
		case NodeType::PASS:						return MALMY_NEW(m_allocator, PassNode)(*this);
		case NodeType::INSTANCE_MATRIX:				return MALMY_NEW(m_allocator, InstanceMatrixNode)(*this);
		case NodeType::FUNCTION:					return MALMY_NEW(m_allocator, FunctionNode)(*this);
		case NodeType::BINARY_FUNCTION:				return MALMY_NEW(m_allocator, BinaryFunctionNode)(*this);
	}
...
void ShaderEditor::onGUIRightColumn()
{
	...
	for(auto* node : nodes)
	{
		...
		{
			execute(MALMY_NEW(m_allocator, MoveNodeCommand)(node->m_id, new_pos, *this));
		}
	}
	...
	if(ImGui::BeginPopup("context_menu"))
	{
		...
		if(m_current_node_id >= 0)
		{
			...
			{
				execute(MALMY_NEW(m_allocator, RemoveNodeCommand)(m_current_node_id, m_current_shader_type, *this));
				m_current_node_id = -1;
			}
		}
		...
		if (ImGui::BeginMenu("Add"))
		{
			for (auto node_type : NODE_TYPES)
			{
				...
				{
					execute(MALMY_NEW(m_allocator, CreateNodeCommand)(
...
void ShaderEditor::newGraph()
{
	...
	m_fragment_nodes.push(MALMY_NEW(m_allocator, FragmentOutputNode)(*this));
	m_fragment_nodes.back()->m_pos.x = 50;
	m_fragment_nodes.back()->m_pos.y = 50;
	m_fragment_nodes.back()->m_id = ++m_last_node_id;

	m_vertex_nodes.push(MALMY_NEW(m_allocator, PositionOutputNode)(*this));
