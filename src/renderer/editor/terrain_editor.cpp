...
struct PaintTerrainCommand MALMY_FINAL : public IEditorCommand
{
	struct Rectangle
	{
		int from_x;
...
private:
	...
	void applyData(Array<u8>& data)
	{
		...
		{
			IScene* scene = m_world_editor.getProject()->getScene(crc32("physics"));
			if (!scene) return;
...
{
	m_world_editor.projectDestroyed().unbind<TerrainEditor, &TerrainEditor::onProjectDestroyed>(this);
	if (m_brush_texture)
	{
		m_brush_texture->destroy();
		MALMY_DELETE(m_world_editor.getAllocator(), m_brush_texture);
	}
...
{
	return MALMY_NEW(editor.getAllocator(), PaintTerrainCommand)(editor);
}
	...
	, m_y_spread(0, 0)
{
	...
	m_increase_brush_size = MALMY_NEW(editor.getAllocator(), Action)("Increase brush size", "Terrain editor - Increase brush size", "increaseBrushSize");
	m_increase_brush_size->is_global = false;
	m_increase_brush_size->func.bind<TerrainEditor, &TerrainEditor::increaseBrushSize>(this);
	m_decrease_brush_size = MALMY_NEW(editor.getAllocator(), Action)("Decrease brush size", "Terrain editor - decrease brush size", "decreaseBrushSize");
	...
	m_increase_texture_idx = MALMY_NEW(editor.getAllocator(), Action)("Next terrain texture", "Terrain editor - next texture", "nextTerrainTexture");
	...
	m_decrease_texture_idx = MALMY_NEW(editor.getAllocator(), Action)("Previous terrain texture", "Terrain editor - previous texture", "prevTerrainTexture");
	...
	m_smooth_terrain_action = MALMY_NEW(editor.getAllocator(), Action)("Smooth terrain", "Terrain editor - smooth", "smoothTerrain");
	m_smooth_terrain_action->is_global = false;
	m_lower_terrain_action = MALMY_NEW(editor.getAllocator(), Action)("Lower terrain", "Terrain editor - lower", "lowerTerrain");
	...
	m_remove_grass_action =
		MALMY_NEW(editor.getAllocator(), Action)("Remove grass from terrain", "Terrain editor - remove grass", "removeGrassFromTerrain");
	...
	editor.projectDestroyed().bind<TerrainEditor, &TerrainEditor::onProjectDestroyed>(this);
}
		...
		if (ImGui::Button("Select mask"))
		{
			...
			if (PlatformInterface::getOpenFilename(filename, lengthOf(filename), "All\0*.*\0", nullptr))
			{
				...
				if (data)
				{
					...
					m_brush_texture = MALMY_NEW(m_world_editor.getAllocator(), Texture)(
...
{
	PaintTerrainCommand* command = MALMY_NEW(m_world_editor.getAllocator(), PaintTerrainCommand)(m_world_editor,
		action_type,
