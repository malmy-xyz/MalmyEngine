	...
	, m_deferred_prefab_inserts(app.getWorldEditor().getAllocator())
{
	...
	m_editor.projectCreated().bind<SceneView, &SceneView::onProjectCreated>(this);
	m_editor.projectDestroyed().bind<SceneView, &SceneView::onProjectDestroyed>(this);

	m_toggle_gizmo_step_action =
		MALMY_NEW(allocator, Action)("Enable/disable gizmo step", "Enable/disable gizmo step", "toggleGizmoStep");
	m_toggle_gizmo_step_action->is_global = false;
	m_app.addAction(m_toggle_gizmo_step_action);

	m_move_forward_action = MALMY_NEW(allocator, Action)("Move forward", "Move camera forward", "moveForward");
	m_move_forward_action->is_global = false;
	m_app.addAction(m_move_forward_action);

	m_move_back_action = MALMY_NEW(allocator, Action)("Move back", "Move camera back", "moveBack");
	m_move_back_action->is_global = false;
	m_app.addAction(m_move_back_action);

	m_move_left_action = MALMY_NEW(allocator, Action)("Move left", "Move camera left", "moveLeft");
	m_move_left_action->is_global = false;
	m_app.addAction(m_move_left_action);

	m_move_right_action = MALMY_NEW(allocator, Action)("Move right", "Move camera right", "moveRight");
	m_move_right_action->is_global = false;
	m_app.addAction(m_move_right_action);

	m_move_up_action = MALMY_NEW(allocator, Action)("Move up", "Move camera up", "moveUp");
	m_move_up_action->is_global = false;
	m_app.addAction(m_move_up_action);

	m_move_down_action = MALMY_NEW(allocator, Action)("Move down", "Move camera down", "moveDown");
	m_move_down_action->is_global = false;
	m_app.addAction(m_move_down_action);

	m_camera_speed_action = MALMY_NEW(allocator, Action)("Camera speed", "Reset camera speed", "cameraSpeed");
...
SceneView::~SceneView()
{
	...
	m_editor.projectCreated().unbind<SceneView, &SceneView::onProjectCreated>(this);
	m_editor.projectDestroyed().unbind<SceneView, &SceneView::onProjectDestroyed>(this);
...
void SceneView::onProjectCreated()
{
	IScene* scene = m_editor.getProject()->getScene(crc32("renderer"));
	m_pipeline->setScene((RenderScene*)scene);
}


void SceneView::onProjectDestroyed()
{
...
void SceneView::renderSelection()
{
	...
	for (Entity e : entities)
	{
		...
		Matrix mtx = project.getMatrix(e);
		if (model)
		{
