...
{
	m_task = MALMY_NEW(m_editor.getAllocator(), Task)(*this);
...
bool ShaderCompiler::getSourceFromBinaryBasename(char* out, int max_size, const char* binary_basename)
{
	...
	*cout = '\0';
	...
	//burda hata geliyo shader conpile edilemiyor demkki
...
ShaderCompiler::~ShaderCompiler()
{
	...
	m_semaphore.signal();
	m_task->destroy();
	MALMY_DELETE(m_app.getWorldEditor().getAllocator(), m_task);
