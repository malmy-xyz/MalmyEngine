...
struct FontPlugin MALMY_FINAL : public AssetBrowser::IPlugin
{
...
struct MaterialPlugin MALMY_FINAL : public AssetBrowser::IPlugin
{
...
struct ModelPlugin MALMY_FINAL : public AssetBrowser::IPlugin
{
	struct Task : MT::Task {
	...
	~ModelPlugin()
	{
		...
		engine.destroyProject(*m_project);
		Pipeline::destroy(m_pipeline);
		engine.destroyProject(*m_tile.project);
	...
	void createTextureTileTask()
	{
		while (!m_texture_tile_creator.shutdown)
		{
			...
			u32 hash = crc32(tile);
			StaticString<MAX_PATH_LENGTH> out_path(".malmy/asset_tiles/", hash, ".dds");
	...
	void createTileProject()
	{
		Engine& engine = m_app.getWorldEditor().getEngine();
		m_tile.project = &engine.createProject(false);
	...
	void createPreviewProject()
	{
		auto& engine = m_app.getWorldEditor().getEngine();
		m_project = &engine.createProject(false);
	...
	void update() override
	{
		if (m_tile.frame_countdown >= 0)
		{
			...
			{
				StaticString<MAX_PATH_LENGTH> path(".malmy/asset_tiles/", m_tile.path_hash, ".dds");
...
struct TexturePlugin MALMY_FINAL : public AssetBrowser::IPlugin
{
...
struct ShaderPlugin MALMY_FINAL : public AssetBrowser::IPlugin
{
...
struct EnvironmentProbePlugin MALMY_FINAL : public PropertyGrid::IPlugin
{
	...
	bool saveCubemap(ComponentUID cmp, const u8* data, int texture_size, const char* postfix)
	{
		...
		StaticString<MAX_PATH_LENGTH> path(base_path, "projects/", m_app.getWorldEditor().getProject()->getName());
	...
	void generateCubemap(ComponentUID cmp)
	{
		...
		Project* project = m_app.getWorldEditor().getProject();
		if (project->getName()[0] == '\0')
		{
			g_log_error.log("Editor") << "Project must be saved before environment probe can be generated.";
			return;
		}
...
struct EmitterPlugin MALMY_FINAL : public PropertyGrid::IPlugin
{
...
struct TerrainPlugin MALMY_FINAL : public PropertyGrid::IPlugin
{
	explicit TerrainPlugin(StudioApp& app)
		: m_app(app)
	{
		WorldEditor& editor = app.getWorldEditor();
		m_terrain_editor = MALMY_NEW(editor.getAllocator(), TerrainEditor)(editor, app);
	}


	~TerrainPlugin()
	{
		MALMY_DELETE(m_app.getWorldEditor().getAllocator(), m_terrain_editor);
	}
...
struct FurPainter MALMY_FINAL : public WorldEditor::Plugin
{
...
struct RenderInterfaceImpl MALMY_FINAL : public RenderInterface
{
		...
		, m_textures(editor.getAllocator())
	{
		...
		editor.projectCreated().bind<RenderInterfaceImpl, &RenderInterfaceImpl::onProjectCreated>(this);
		editor.projectDestroyed().bind<RenderInterfaceImpl, &RenderInterfaceImpl::onProjectDestroyed>(this);
	}
	...
	~RenderInterfaceImpl()
	{
		...
		m_editor.projectCreated().unbind<RenderInterfaceImpl, &RenderInterfaceImpl::onProjectCreated>(this);
		m_editor.projectDestroyed().unbind<RenderInterfaceImpl, &RenderInterfaceImpl::onProjectDestroyed>(this);
	}
	...
	ImFont* addFont(const char* filename, int size) override
	{
		...
		Texture* texture = MALMY_NEW(engine.getAllocator(), Texture)(
		...
		{
			old_texture->destroy();
			MALMY_DELETE(engine.getAllocator(), old_texture);
		}
	...
	ImTextureID createTexture(const char* name, const void* pixels, int w, int h) override
	{
		...
		Texture* texture = MALMY_NEW(allocator, Texture)(Path(name), *rm.get(Texture::TYPE), allocator);
	...
	void destroyTexture(ImTextureID handle) override
	{
		...
		texture->destroy();
		MALMY_DELETE(allocator, texture);
	}
	...
	{
		m_render_scene = static_cast<RenderScene*>(m_editor.getProject()->getScene(MODEL_INSTANCE_TYPE));
	}
...
struct EditorUIRenderPlugin MALMY_FINAL : public StudioApp::GUIPlugin
{
		...
		, m_engine(app.getWorldEditor().getEngine())
	{
		...
		Texture* texture = MALMY_NEW(editor.getAllocator(), Texture)(
		...
		{
			old_texture->destroy();
			MALMY_DELETE(m_engine.getAllocator(), old_texture);
		}

		IAllocator& allocator = editor.getAllocator();
		RenderInterface* render_interface = MALMY_NEW(allocator, RenderInterfaceImpl)(editor, *scene_view.getPipeline());
	...
	void shutdownImGui()
	{
		...
		texture->destroy();
		MALMY_DELETE(m_app.getWorldEditor().getAllocator(), texture);
...
struct AddTerrainComponentPlugin MALMY_FINAL : public StudioApp::IAddComponentPlugin
{
...
struct StudioAppPlugin : StudioApp::IPlugin
{
		...
		: m_app(app)
	{
		...
		m_font_plugin = MALMY_NEW(allocator, FontPlugin)(app);
		m_texture_plugin = MALMY_NEW(allocator, TexturePlugin)(app);
		m_shader_plugin = MALMY_NEW(allocator, ShaderPlugin)(app);
		AssetBrowser& asset_browser = app.getAssetBrowser();
		...
		m_emitter_plugin = MALMY_NEW(allocator, EmitterPlugin)(app);
		m_env_probe_plugin = MALMY_NEW(allocator, EnvironmentProbePlugin)(app);
		m_terrain_plugin = MALMY_NEW(allocator, TerrainPlugin)(app);
		...
		m_scene_view = MALMY_NEW(allocator, SceneView)(app);
	...
	~StudioAppPlugin()
	{
		...
		MALMY_DELETE(allocator, m_model_plugin);
		MALMY_DELETE(allocator, m_material_plugin);
		MALMY_DELETE(allocator, m_font_plugin);
		MALMY_DELETE(allocator, m_texture_plugin);
		MALMY_DELETE(allocator, m_shader_plugin);
		...
		MALMY_DELETE(allocator, m_emitter_plugin);
		MALMY_DELETE(allocator, m_env_probe_plugin);
		MALMY_DELETE(allocator, m_terrain_plugin);
