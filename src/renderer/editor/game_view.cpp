	...
	, m_editor(app.getWorldEditor())
{
	...
	Action* action = MALMY_NEW(editor.getAllocator(), Action)("Game View", "Toggle game view", "game_view");
	action->func.bind<GameView, &GameView::onAction>(this);
	action->is_selected.bind<GameView, &GameView::isOpen>(this);
	app.addWindowAction(action);

	Action* fullscreen_action = MALMY_NEW(editor.getAllocator(), Action)("Game View fullscreen", "Game View fullscreen", "game_view_fullscreen");
	...
	editor.projectCreated().bind<GameView, &GameView::onProjectCreated>(this);
...
void GameView::onProjectCreated()
{
	auto* scene = m_editor.getProject()->getScene(crc32("renderer"));
	m_pipeline->setScene(static_cast<RenderScene*>(scene));
}


void GameView::onProjectDestroyed()
{
