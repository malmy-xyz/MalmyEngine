...
struct PipelineImpl MALMY_FINAL : public Pipeline
{
	...
	void cleanup()
	{
		...
		{
			MALMY_DELETE(m_allocator, m_framebuffers[i]);
			if (m_framebuffers[i] == m_default_framebuffer) m_default_framebuffer = nullptr;
		}
		MALMY_DELETE(m_allocator, m_default_framebuffer);
	...
	void applyCamera(const char* slot)
	{
		...
		Matrix view = project.getMatrix(camera);
		view.fastInverse();
	...
	void renderShadowmap(int split_index)
	{
		...
		Matrix light_mtx = project.getMatrix(light);
		...
		Matrix camera_matrix = project.getMatrix(m_applied_camera);
	...
	void drawQuadExMaterial(float left, float top, float w, float h, float u0, float v0, float u1, float v1, Material* material)
	{
		...
		if (m_applied_camera.isValid())
		{
			...
			Matrix inv_view_matrix = project.getPositionAndRotation(m_applied_camera);
	...
	void renderAll(const Frustum& frustum, bool render_grass, GameObject camera, u64 layer_mask, bool use_occlusion_culling)
	{
		...
		Vec3 lod_ref_point = m_scene->getProject().getPosition(camera);
	...
	void rasterizeOccluders(u64 layer_mask)
	{
		...
		Matrix view = project->getMatrix(m_applied_camera);
		view.fastInverse();
		m_occlusion_buffer.setCamera(view, projection);
		for (auto& meshes : *m_mesh_buffer)
		{
			m_occlusion_buffer.rasterize(project, meshes);
		}
	...
	void setWindowHandle(void* data) override
	{
		m_default_framebuffer =
			MALMY_NEW(m_allocator, FrameBuffer)("default", m_width, m_height, data);
	}
	...
	void renderModel(Model& model, Pose* pose, const Matrix& mtx) override
	{
		...
		Vec3 camera_pos = m_scene->getProject().getPosition(m_applied_camera);
	...
	MALMY_FORCE_INLINE void renderRigidMesh(bgfx::Encoder* encoder, const Matrix& matrix, Mesh& mesh, float depth)
	{
		Material* material = mesh.material;
	...
	MALMY_FORCE_INLINE void renderRigidMeshInstanced(bgfx::Encoder* encoder, InstanceData& data, const Matrix& matrix, const Mesh& mesh)
	{
		if (data.mesh != &mesh)
		{
			if (data.mesh)
	...
	void finishTerrainInstances(int index)
	{
		...
		Vec3 camera_pos =
			m_scene->getProject().getPosition(m_applied_camera);
...
{
	return MALMY_NEW(allocator, PipelineImpl)(renderer, path, defines, allocator);
}


void Pipeline::destroy(Pipeline* pipeline)
{
	MALMY_DELETE(static_cast<PipelineImpl*>(pipeline)->m_allocator, pipeline);
}
...
int addFramebuffer(lua_State* L)
{
	...
	auto* fb = MALMY_NEW(pipeline->m_allocator, FrameBuffer)(decl);
