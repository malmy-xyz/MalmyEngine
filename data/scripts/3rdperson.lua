...
function init()
	cmp = Engine.createComponent(g_project, this, "physical_controller")
end
...
function update(dt)
	...
	if actions.forward or actions.back or actions.left or actions.right then
		...
		Engine.setEntityRotation(g_project, this, {0, 1, 0}, player_yaw);
	end

	Engine.setEntityLocalRotation(g_project, camera_pitch_pivot_entity, makeQuat({-1, 0, 0}, pitch))
	
	Engine.setEntityRotation(g_project, camera_yaw_pivot_entity, {0, 1, 0}, camera_yaw);
