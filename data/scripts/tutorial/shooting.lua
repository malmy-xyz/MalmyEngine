...
function update(time_delta)
    ...
    if actions.shoot then
		actions.shoot = false
		local muzzle_pos = Engine.getEntityPosition(g_project, muzzle)
        local muzzle_rot = Engine.getEntityRotation(g_project, muzzle)
        ...
        if is_hit then
            -- spawn particle
            local instance = Engine.instantiatePrefab(g_engine, g_project, hit_position, prefab)
    ...
    if to_remove > 0 then
        Engine.destroyEntity(g_project, particles[to_remove].entity)
