...
function getRandomSpawnPos()
    ...
    return Engine.getEntityPosition(g_project, spawn_point)
end
...
function processDespawn(time_delta)
    for i,v in ipairs(to_despawn) do
        ...
        if v.time < 0 then
            Engine.destroyEntity(g_project, v.entity)
...
function update(time_delta)
    ...
    local instance = Engine.instantiatePrefab(g_engine, g_project, pos, prefab)
