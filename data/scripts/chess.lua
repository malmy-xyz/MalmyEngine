...
function init()
	...
	for side = 1,2 do
		for i = 1, 8 do
			...
			piece.x = i
			piece.y = j
			piece.entity = Engine.createEntityEx(g_engine, g_project, 
			{
	...
	-- cursor
	cursor.pos = {0, 0, 0}
	cursor.entity = Engine.createEntityEx(g_engine, g_project,
	{
		position = cursor.pos,
		renderable = {
	...
	function destroyPiece(piece)
		...
		local f = function(t, piece)
			...
			if i > 0 then 
				table.remove(t, i)
				Engine.setEntityPosition(g_project, piece.entity, {15, 0, 15})
	...
	function move(piece, x, y)
		...
		if is_valid_move then
			...
			piece.y = y;
			board[y][x] = piece
			Engine.setEntityPosition(g_project, piece.entity, toWorldPos(x, y))
	...
	function click()
		...
		if piece then
			if selected_piece ~= nil then
				Engine.setEntityPosition(g_project, selected_piece.entity, toWorldPos(selected_piece.x, selected_piece.y))
			end
			
			selected_piece = piece
			Engine.setEntityPosition(g_project, selected_piece.entity, toWorldPos(i, j, true))
		end
	end
end
...
function update(dt)
	...
	Engine.setEntityPosition(g_project, cursor.entity, cursor.pos)
end